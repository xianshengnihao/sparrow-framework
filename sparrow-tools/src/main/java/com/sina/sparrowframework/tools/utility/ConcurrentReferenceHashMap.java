...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    public boolean containsKey( Object key) {
        Entry<K, V> entry = getEntryIfAvailable(key);
        return (entry != null && ObjectToolkit.nullSafeEquals(entry.getKey(), key));
    }
    ...
    private V put( final K key,  final V value, final boolean overwriteExisting) {
        return doTask(key, new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.RESIZE) {
            ...
            protected V execute( Reference<K, V> reference,  Entry<K, V> entry,  Entries entries) {
                ...
                Assert.assertTrue(entries != null, "No entries segment");
                entries.add(value);
                return null;
            }
        });
    ...
    public boolean remove(Object key, final Object value) {
        Boolean result = doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_AFTER, TaskOption.SKIP_IF_EMPTY) {
            ...
            protected Boolean execute( Reference<K, V> reference,  Entry<K, V> entry) {
                if (entry != null && ObjectToolkit.nullSafeEquals(entry.getValue(), value)) {
                    if (reference != null) {
                        reference.release();
                    }
                    return true;
    ...
    public boolean replace(K key, final V oldValue, final V newValue) {
        Boolean result = doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.SKIP_IF_EMPTY) {
            @Override
            protected Boolean execute( Reference<K, V> reference,  Entry<K, V> entry) {
                if (entry != null && ObjectToolkit.nullSafeEquals(entry.getValue(), oldValue)) {
            ...
            while (currRef != null) {
                if (currRef.getHash() == hash) {
                    ...
                    if (entry != null) {
                        ...
                        if (ObjectToolkit.nullSafeEquals(entryKey, key)) {
                            return currRef;
                        }
                    }
                }
    ...
    protected static final class Entry<K, V> implements Map.Entry<K, V> {
        ...
        public final boolean equals(Object other) {
            ...
            return (ObjectToolkit.nullSafeEquals(getKey(), otherEntry.getKey()) &&
                    ObjectToolkit.nullSafeEquals(getValue(), otherEntry.getValue()));
        }

        @Override
        public final int hashCode() {
            return (ObjectToolkit.nullSafeHashCode(this.key) ^ ObjectToolkit.nullSafeHashCode(this.value));
        }
    }
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public boolean contains( Object o) {
            if (o != null && o instanceof Map.Entry<?, ?>) {
                ...
                if (other != null) {
                    return ObjectToolkit.nullSafeEquals(entry.getValue(), other.getValue());
                }
            }
            return false;
        }
    ...
    private class EntryIterator implements Iterator<Map.Entry<K, V>> {
        ...
        @Override
        public void remove() {
            Assert.assertTrue(this.last != null, "No element to remove");
