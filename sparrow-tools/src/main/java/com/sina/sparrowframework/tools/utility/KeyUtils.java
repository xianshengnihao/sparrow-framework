...
public abstract class KeyUtils {
    ...
    /**
     * @param keySize AES key 的 key size,参考 {@link Algorithm#AES}
     * @return AES key
     ...
     * 创建 RSA 密钥对
     *
     * @param keySize 参考 {@link Algorithm#RSA}
     * @return RSA 密钥对
     */
    ...
    public static Key readKey(KeyType type, String base64Text) throws Exception {
        Assert.assertNotNull(type, "KeyType can not be null");
        Assert.assertNotNull(base64Text, "The content can not be null");
    ...
    protected static KeySpec getKeySpec(KeyPairType type, boolean privateKey, byte[] encoded) {
        Assert.assertNotNull(type,"KeyPairType can not be null");
        KeySpec spec;
