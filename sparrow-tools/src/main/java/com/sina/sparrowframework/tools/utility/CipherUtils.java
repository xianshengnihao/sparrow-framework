...
public abstract class CipherUtils {
    ...
    public static byte[] decrypt(String keyBase64, Algorithm algorithm, byte[] cipherContent) throws Exception {
        Assert.assertTrue(algorithm.algorithm.startsWith("AES"), "algorithm[%s] not use AES", algorithm.algorithm);
    ...
    public static byte[] decryptWithRsaPublic(String keyBase64, Algorithm algorithm, byte[] cipherContent) throws Exception {
        Assert.assertTrue(algorithm.algorithm.startsWith("RSA"), "algorithm[%s] not use RSA", algorithm.algorithm);
    ...
    public static File encrypt(Key key, Algorithm algorithm, InputStream in) throws Exception {
        File file = new File(System.getProperty("java.io.tmpdir"), UUID.randomUUID().toString());
        if (!file.exists() && file.createNewFile()) {
            LOG.info("创建临时文件,路径:{}", file.getAbsolutePath());
        }
        encrypt(key, algorithm, in, new FileOutputStream(file));
        return file;
    }
