...
public class WjAccountClient {
     ...
     * URL参数解密
     */
    public static String decryptUrlParam(String encKey, String encData) {
        try {
            String key = CipherUtils.decryptWithRsaPublic128(
                    financePartnerPublicKeyStr
                    , CipherUtils.Algorithm.RSA, encKey);
            String data = CipherUtils.decryptByAesEcbPkcs5paddingAes(
                    encData, key);
            return data;
        } catch (Exception e) {
            logger.error("\r\n,金融账户URL参数解密异常", e);
            throw new BizFailException(BaseCode.THIRD_REMOTE_ERROR, "金融账户URL参数解密异常");
        }
    }
